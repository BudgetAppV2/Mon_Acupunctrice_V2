...
export default function ProfilPage() {
  ...
  const recentPublished = published.sort((a, b) => (b.publishedAt?.toMillis() ?? 0) - (a.publishedAt?.toMillis() ?? 0)).slice(0, 10);
    ...
    <div className="min-h-screen bg-sand">
      ...
      <div className="px-4 pt-6 pb-4">
        <div className="flex items-center gap-3 mb-6">
          {user?.photoURL ? (
            <img src={user.photoURL} alt="" className="w-14 h-14 rounded-full" referrerPolicy="no-referrer" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          ) : (
            <div className="w-14 h-14 rounded-full bg-sage/20 flex items-center justify-center text-sage text-xl font-bold">{user?.displayName?.charAt(0) || '?'}</div>
          )}
          <div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          <div className="bg-white rounded-xl p-3 text-center">
            <CheckCircleIcon className="w-5 h-5 text-green-500 mx-auto mb-1" /><p className="text-lg font-bold text-gray-900">{published.length}</p><p className="text-[10px] text-gray-500">Publiees</p>
          </div>
          <div className="bg-white rounded-xl p-3 text-center">
            <ClockIcon className="w-5 h-5 text-blue-500 mx-auto mb-1" /><p className="text-lg font-bold text-gray-900">{scheduled.length}</p><p className="text-[10px] text-gray-500">Planifiees</p>
          </div>
          <div className="bg-white rounded-xl p-3 text-center">
        ...
        <div className="space-y-1.5 mb-6">
          {customCategories.map(cat => (
            <div key={cat} className="flex items-center justify-between bg-white rounded-lg px-3 py-2">
              ...
              <button onClick={() => updateCustomCategories(customCategories.filter(c => c !== cat))}><XMarkIcon className="w-4 h-4 text-gray-400" /></button>
            </div>
          ))}
          <div className="flex gap-2">
            <input value={newCat} onChange={e => setNewCat(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCategory()} placeholder="Nouvelle categorie" className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-sm" />
            <button onClick={addCategory} disabled={!newCat.trim()} className="flex items-center gap-1 px-3 py-1.5 bg-sage text-white rounded-lg text-sm disabled:opacity-50"><PlusIcon className="w-3.5 h-3.5" /></button>
          </div>
            ...
            {recentPublished.map(item => (
              <div key={item.id} className="flex items-center gap-3 bg-white rounded-xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.publishedAt && <p className="text-[10px] text-gray-500">{item.publishedAt.toDate().toLocaleDateString('fr-CA', { day: 'numeric', month: 'short', year: 'numeric' })}</p>}
                </div>
