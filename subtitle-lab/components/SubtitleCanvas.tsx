...
export default function SubtitleCanvas() {
  ...
  useEffect(() => {
    ...
    const loop = (wallMs: number) => {
      ...
      if (canvas) {
        renderFrame({ canvas, blocks: blocksRef.current, globalPreset: presetRef.current, currentMs: timeRef.current, nowMs: wallMs, canvasWidth: CANVAS_W, canvasHeight: CANVAS_H });
      }
  ...
  return (
    <canvas
      ...
      style={{ maxWidth: '100%', cursor: isDragging ? 'grabbing' : 'grab', touchAction: 'none' }}
